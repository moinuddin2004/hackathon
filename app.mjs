...
const firebaseConfig = {
  ...
  document.getElementById('postSubmit').addEventListener('click', async () =>{
    ...
    const thehead = document.getElementById('headBox').value;
    const user = auth.currentUser;
      ...
      querySnapshot.forEach ((snapshot) =>{
        const data = snapshot.data();
            const user = auth.currentUser;
             ...
             Logged in as: <a href="#">${data.firstname+data.lastname}</a>
           `;
           ...
           if (data.timestamp) {
               ...
               const formattedTimestamp = timestamp.toLocaleString(); // Format the Date object as a human-readable string
           
               bookDiv.innerHTML = `
               <div class="classtitle" id="book${snapshot.id}">
               <h3>${data.head}</h3>
              
              <a href="./profile.html"> <h3>${data.firstname} ${data.lastname}</h3></a>
               <span> ${formattedTimestamp} </span>
               <p>${data.post} </p>
               <button id="del"    onclick="deletePost('${snapshot.id}')"  ref="${snapshot.id}   class="delete-button">Delete</button>
               <button   id="upd"  onclick="editPost('${snapshot.id}', '${data.post}', '${data.head}')">Update</button>
...
///// Add event listener for delete post button ref="${data.useremail}"

              //  <a href="#"  id="profileLink">${data.firstname+data.lastname} </a>
...
// Edit Book function
window.editPost = async (docId, post,heading) => {
  console.log("hi");
  ...
  <label for="head">Heading:</label>
   <textarea name="heading" id="head">${heading}</textarea>
 <br>
...
window.completeEdit = async (e, docId) => {
  ...
  console.log("e.target.author: ", e.target.heading.value);
  console.log("e.target.author: ", e.target.post.value);
...
//// onAuth's else
      ...
      Please <a href="./signin.html">Login</a> to view posts
      </center>
      `
